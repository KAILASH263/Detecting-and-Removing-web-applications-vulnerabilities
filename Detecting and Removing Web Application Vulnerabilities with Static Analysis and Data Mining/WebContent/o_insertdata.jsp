						...
						if ( rs1.next() )
						   {
							...
							response.sendRedirect("o_register.jsp");
					   }
					   else
					   {
							PreparedStatement ps=connection.prepareStatement("insert into owner(username,password,email,mobile,dob,gender,address,pincode,status,image,acc_type) values(?,?,?,?,?,?,?,?,?,?,?)");
